...
export const careerContent: ReactNode = (
	<ol className="relative space-y-6 before:pointer-events-none before:absolute before:top-2 before:bottom-2 before:left-[0.72rem] before:w-px before:bg-gradient-to-b before:from-border-card before:via-border-subtle before:to-transparent before:content-[''] md:before:left-[10.6rem]">
		<li className="relative grid gap-3 pl-7 md:grid-cols-[10rem_1fr] md:gap-x-5 md:pl-0">
			<span className="absolute left-[0.35rem] top-3 h-3 w-3 rounded-full border border-border-card bg-text-primary shadow-[0_0_0_4px_var(--color-surface-card)] md:left-[10.25rem]" />
			<div className="rounded-xl border border-border-card bg-surface-card px-4 py-3 shadow-[inset_0_1px_0_var(--color-border-subtle)]">
			...
			<div className="rounded-xl border border-border-subtle bg-surface-card px-4 py-3 md:col-start-2">
				<p className="text-[11px] font-bold tracking-[0.14em] text-text-sub sm:text-xs">学業実績</p>
				<ul className="mt-2 list-disc space-y-1.5 pl-5 text-text-sub marker:text-text-sub">
